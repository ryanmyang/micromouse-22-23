...
int16_t ir_left_reading = 0;
int16_t ir_right_reading = 0;
int16_t ir_fright_reading = 0;
int16_t ir_fleft_reading = 0;
...
/*  move(2);
  ...
  while (1)
  {
	ir_fleft_reading = readFrontLeftIR();
	ir_fright_reading = readFrontRightIR();
	if (((ir_fleft_reading + ir_fright_reading)/ 2) <= 350){
		move(1);
	}
	else{
		ir_right_reading = readRightIR();
		if (ir_right_reading > 350){
			turn(-1);
		}
		else{
			turn(1);
